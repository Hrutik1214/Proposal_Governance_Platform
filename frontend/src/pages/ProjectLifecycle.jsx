...
const getApiUrl = () => {
  if (typeof window !== 'undefined') {
    if (window.location.port === '5173' || window.location.port === '3000') {
      return `http://${window.location.hostname}:5031/api`;
    }
    return '/api';
  }
  return 'http://localhost:5031/api';
};
...
const inputStyle = {
  ...
  borderRadius: '10px', color: '#000000', padding: '0.65rem 0.9rem', fontSize: '0.88rem',
...
const btnSuccess = { ...btnPrimary, background: 'var(--color-approved)' };
const btnDanger  = { ...btnPrimary, background: 'var(--color-rejected)' };
const btnSecondary = { ...btnPrimary, background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', color: '#000000' };
const labelStyle = { display: 'block', fontSize: '0.78rem', color: '#1E293B', marginBottom: '0.3rem', fontWeight: 600 };
...
export default function ProjectLifecycle({ proposalId, proposalTitle, proposalStatus, onBack }) {
  ...
  async function addMilestone() {
    const dateVal = validateFutureDate(mlForm.targetDate, 'Target Date');
    if (!dateVal.isValid) {
      showToast(dateVal.message, 'error');
      return;
    }
    setLoading(true);
