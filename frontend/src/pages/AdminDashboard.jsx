...
export default function AdminDashboard() {
 ...
 const fetchDashboardData = async () => {
  ...
  useEffect(() => {
    ...
    // Fetch potential reviewers from live database API
    const loadReviewers = async () => {
      try {
        const revs = await api.get('/proposals/reviewers');
                ...
                <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                  ...
                  {selectedProposal.supportingDocumentPath && (
                    <button
                      ...
                      style={{ flex: 1, justifyContent: 'center', fontSize: '0.78rem', padding: '0.4rem 0.6rem', minWidth: '120px' }}
                    >
                      📄 Open Document
                    </button>
                  )}
