...
export default function FounderDashboard({ user, setUser, currentTab, setCurrentTab }) {
  ...
  const handleViewProposal = async (proposal) => {
    ...
    try {
      ...
      if (proposal.status !== 'Draft' && proposal.status !== 'Submitted') {
        const revData = await api.get(`/reviews/proposal/${proposal.id}`).catch(() => []);
  ...
  const handleCreateOrUpdateProposal = async (e) => {
    ...
    if (startupName.trim().length < 2 || startupName.trim().length > 30) {
      setFormError('Startup Name must be between 2 and 30 characters long.');
      return;
    }

    if (title.trim().length < 2 || title.trim().length > 30) {
      setFormError('Proposal Title must be between 2 and 30 characters long.');
      return;
    }

    const reqAmt = parseFloat(requestedAmount);
    if (isNaN(reqAmt) || reqAmt <= 0) {
      setFormError('Requested Funding Amount must be a positive number greater than zero.');
      return;
    }

    const eqOffered = parseFloat(equityOffered);
    if (isNaN(eqOffered) || eqOffered < 0 || eqOffered > 100) {
    ...
    if (demoVideoUrl && demoVideoUrl.trim()) {
      try {
        const parsed = new URL(demoVideoUrl.trim());
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          setFormError('Demo Video URL must start with http:// or https://');
          return;
        }
      } catch {
        setFormError('Please enter a valid Demo Video URL (e.g. https://youtube.com/watch?v=...).');
        return;
      }
    }
    ...
    const payload = {
      ...
      proposedStatement: proposedStatement.trim(),
      equityOffered: eqOffered,
      businessModel: businessModel ? businessModel.trim() : '',
  ...
  // Load state if navigating to edit-proposal with state
  useEffect(() => {
    if (currentTab === 'edit-proposal' && location.state?.proposal) {
      const p = location.state.proposal;
      setSelectedProposal(p);
      setTitle(p.title || '');
      setDescription(p.description || '');
      setRequestedAmount(p.requestedAmount != null ? p.requestedAmount.toString() : '');
      setUploadedFilePath(p.supportingDocumentPath || '');
      setUploadedFileName(p.supportingDocumentPath ? p.supportingDocumentPath.split('/').pop() : '');
      
      setStartupName(p.startupName || '');
      setProblemStatement(p.problemStatement || '');
      setProposedStatement(p.proposedStatement || '');
      setEquityOffered(p.equityOffered != null ? p.equityOffered.toString() : '');
      setBusinessModel(p.businessModel || '');
      setTeamDetails(p.teamDetails || '');
      setDemoVideoUrl(p.demoVideoUrl || '');
    }
  }, [currentTab, location.state]);
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              <div className="form-group">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>{startupName.length}/30 chars</span>
                </div>
                <input type="text" maxLength={30} className="form-input" value={startupName} onChange={(e) => setStartupName(e.target.value)} placeholder="E.g. Acme Analytics (max 30 chars)" />
              </div>
              <div className="form-group">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <label>Proposal Title *</label>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>{title.length}/30 chars</span>
                </div>
                <input type="text" maxLength={30} className="form-input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="E.g. Machine Learning Infra (max 30 chars)" />
              </div>
            </div>
                  ...
                  <tbody>
                    {proposals.map((prop) => (
                      <tr key={prop.id} onClick={() => handleViewProposal(prop)} style={{ cursor: 'pointer', background: selectedProposal?.id === prop.id ? 'rgba(255,255,255,0.02)' : 'transparent' }}>
                        <td style={{ fontWeight: '500' }}>{prop.title || 'Untitled Proposal'}</td>
                        <td style={{ fontFamily: 'var(--font-mono)' }}>{formatCurrency(prop.requestedAmount)}</td>
                        <td>
                          <span className={`badge ${getStatusBadgeClass(prop.status)}`}>{prop.status || 'Draft'}</span>
                        </td>
                        <td style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{prop.updatedAt ? new Date(prop.updatedAt).toLocaleDateString() : '—'}</td>
                        <td>
              ...
              <div className="detail-card" style={{ animation: 'fadeIn 0.3s ease-out' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
                  <div style={{ background: 'rgba(6,182,212,0.07)', border: '1px solid rgba(6,182,212,0.18)', borderRadius: '8px', padding: '0.75rem', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '1rem', fontWeight: '700', fontFamily: 'var(--font-mono)', color: 'var(--accent-cyan)' }}>{formatCurrency(selectedProposal.requestedAmount)}</div>
                  </div>
                  ...
                  {(selectedProposal.approvedAmount || 0) > 0 ? (
                    <div style={{ background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(16,185,129,0.18)', borderRadius: '8px', padding: '0.75rem', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '1rem', fontWeight: '700', fontFamily: 'var(--font-mono)', color: 'var(--accent-secondary)' }}>{formatCurrency(selectedProposal.approvedAmount)}</div>
                    </div>
                  ) : (
                ...
                {selectedProposal.supportingDocumentPath && (
                  <div style={{ marginBottom: '0.75rem' }}>
                    ...
                    <button
                      type="button"
                      onClick={() => api.downloadFile(selectedProposal.supportingDocumentPath, `${selectedProposal.title || 'proposal'}-document.pdf`)}
                      className="btn btn-secondary"
                ...
                {reviews.length > 0 && (
                  <div className="detail-section" style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
                    ...
                    {reviews.map((rev) => {
                      const scoreSum = (Number(rev.feasibilityScore) || 0) + (Number(rev.strategicScore) || 0) + (Number(rev.riskScore) || 0) + (Number(rev.roiScore) || 0);
                      const avgScore = (scoreSum / 4).toFixed(1);
                      return (
                        <div key={rev.id} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', padding: '0.85rem', borderRadius: '6px', marginBottom: '0.5rem', fontSize: '0.85rem' }}>
                          <div className="flex-between mb-1">
                            <strong>{rev.reviewer?.fullName || 'Anonymous Reviewer'}</strong>
                            <span style={{ color: 'var(--accent-cyan)' }}>Avg Score: {avgScore}/10</span>
                          </div>
                ...
                {capitalAllocation && (() => {
                  const alloc = Number(capitalAllocation.allocatedAmount) || 0;
                  const disb = Number(capitalAllocation.disbursedAmount) || 0;
                  const utilPct = alloc > 0 ? Math.min(100, Math.round((disb / alloc) * 100)) : 0;
                  return (
                    <div className="detail-section" style={{ marginTop: '1.5rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '0.85rem', marginBottom: '1rem' }}>
                        <div>
                          ...
                          <strong style={{ fontFamily: 'var(--font-mono)' }}>{formatCurrency(alloc)}</strong>
                        </div>
                        <div>
                          <span style={{ color: 'var(--text-secondary)' }}>Disbursed:</span>{' '}
                          <strong style={{ fontFamily: 'var(--font-mono)' }}>{formatCurrency(disb)}</strong>
                        </div>
                      </div>
                      ...
                      <div style={{ marginBottom: '1rem' }}>
                        ...
                        <div className="progress-container">
                          <div className="progress-bar cyan" style={{ width: `${utilPct}%` }}></div>
                        </div>
                      </div>
                  ...
                  <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ margin: '0.25rem 0 0 0', fontWeight: '500', color: isApp ? 'var(--color-underreview)' : 'var(--text-primary)', fontSize: '0.82rem' }}>
