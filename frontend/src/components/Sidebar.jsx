...
export default function Sidebar({ user, currentTab, setCurrentTab, sidebarOpen, hasPremium }) {
  if (!user) return null;
  ...
  const defaultDashboardTab = user?.role === 'Investor' ? 'marketplace' : user?.role === 'Reviewer' ? 'reviews' : 'overview';

  return (
    <aside className={`sidebar${sidebarOpen ? ' mobile-open' : ''}`} role="navigation" aria-label="Main navigation">
      {/* Logo */}
      <div className="sidebar-logo" onClick={() => setCurrentTab(defaultDashboardTab)} style={{ cursor: 'pointer' }} title="Go to Dashboard">
      ...
      <ul className="sidebar-menu">
        ...
        {user.role === 'Admin' && (
          <>
            ...
            <NavItem tab="verification-review" icon={ICONS.verifyFounders} label="Verification"         id="nav-verification-center" />
      ...
      <div className="sidebar-user">
        <div className="user-badge">
          ...
          <div className="user-info">
            <h4 title={user.fullName} style={{ color: '#000000', fontWeight: 700 }}>{user.fullName || user.username}</h4>
            <div className="user-role-chip" style={{ color: '#000000', fontWeight: 700 }}>{user.role}</div>
            {user.department && (
              <div style={{ fontSize: '0.68rem', color: '#1E293B', fontWeight: 600, marginTop: '2px' }}>
